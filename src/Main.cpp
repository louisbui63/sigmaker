...
static bool GetOperandOffsetARM(const insn_t &instruction,
                                uint8_t *operandOffset,
                                uint8_t *operandLength) {

  // Iterate all operands
  for (const auto &op : instruction.ops) {
    // For ARM, we have to filter a bit though, only wildcard those operand
    // types
    switch (op.type) {
    case o_mem:
    case o_far:
...
static bool GetOperand(const insn_t &instruction, uint8_t *operandOffset,
                       uint8_t *operandLength, uint32_t operandTypeBitmask) {
...
static std::vector<ea_t> FindSignatureOccurences(std::string_view idaSignature,
  ...
  while (true) {
    auto occurrence = bin_search(ea, inf_get_max_ea(), binaryPattern,
...
static std::expected<Signature, std::string> GenerateUniqueSignatureForEA(
    ea_t ea, bool wildcardOperands, bool continueOutsideOfFunction,
    uint32_t operandTypeBitmask, size_t maxSignatureLength = 1000,
    bool askLongerSignature = true) {
  if (ea == BADADDR) {
  ...
  while (true) {
    ...
    if (sigPartLength > maxSignatureLength) {
      if (askLongerSignature) {
        auto result =
            ask_yn(ASKBTN_YES, "Signature is already at %llu bytes. Continue?",
                   signature.size());
        if (result == 1) { // Yes
          sigPartLength = 0;
        } else if (result == 0) { // No
    ...
    if (wildcardOperands &&
        GetOperand(instruction, &operandOffset, &operandLength,
                   operandTypeBitmask) &&
        operandLength > 0) {
      // Add opcodes
      AddBytesToSignature(signature, currentAddress, operandOffset, false);
      // Wildcards for operands
      AddBytesToSignature(signature, currentAddress + operandOffset,
                          operandLength, true);
      // If the operand is on the "left side", add the operator from the "right
      // side"
      if (operandOffset == 0) {
        AddBytesToSignature(signature, currentAddress + operandLength,
                            currentInstructionLength - operandLength, false);
      }
    } else {
      // No operand, add all bytes
      AddBytesToSignature(signature, currentAddress, currentInstructionLength,
                          false);
    }
    ...
    if (!continueOutsideOfFunction && currentFunction &&
        get_func(currentAddress) != currentFunction) {
...
static std::expected<Signature, std::string>
GenerateSignatureForEARange(ea_t eaStart, ea_t eaEnd, bool wildcardOperands,
                            uint32_t operandTypeBitmask) {
  ...
  while (true) {
    ...
    if (currentInstructionLength <= 0) {
      ...
      if (currentAddress < eaEnd) {
        AddBytesToSignature(signature, currentAddress, eaEnd - currentAddress,
                            false);
      }
      TrimSignature(signature);
      return signature;
...
void PrintSignatureForEA(const std::expected<Signature, std::string> &signature,
                         ea_t ea, SignatureType sigType) {
...
static void FindXRefs(ea_t ea, bool wildcardOperands,
                      bool continueOutsideOfFunction,
                      std::vector<std::tuple<ea_t, Signature>> &xrefSignatures,
                      size_t maxSignatureLength, uint32_t operandTypeBitmask) {
  xrefblk_t xref{};

  // Count code xrefs
  size_t xrefCount = 0;
  for (auto xref_ok = xref.first_to(ea, XREF_FAR); xref_ok;
  ...
  for (auto xref_ok = xref.first_to(ea, XREF_FAR); xref_ok;
    ...
    replace_wait_box(
        "Processing xref %llu of %llu (%0.1f%%)...\n\nSuitable Signatures: "
        "%llu\nShortest Signature: %llu Bytes",
        i + 1, xrefCount, (static_cast<float>(i) / xrefCount) * 100.0f,
        xrefSignatures.size(),
        (shortestSignatureLength <= maxSignatureLength ? shortestSignatureLength
                                                       : 0));

    // Genreate signature for xref
    auto signature = GenerateUniqueSignatureForEA(
        xref.from, wildcardOperands, continueOutsideOfFunction,
        operandTypeBitmask, maxSignatureLength, false);
    if (!signature.has_value()) {
      continue;
    }
  ...
  std::ranges::sort(xrefSignatures, [](const auto &a, const auto &b) -> bool {
    return std::get<1>(a).size() < std::get<1>(b).size();
  });
}

static void PrintXRefSignaturesForEA(
    ea_t ea, const std::vector<std::tuple<ea_t, Signature>> &xrefSignatures,
    SignatureType sigType, size_t topCount) {
...
static void PrintSelectedCode(ea_t start, ea_t end, SignatureType sigType,
                              bool wildcardOperands, uint32_t operandBitmask) {
  const auto selectionSize = end - start;
  // Create signature of fixed size from selection

  auto signature =
      GenerateSignatureForEARange(start, end, wildcardOperands, operandBitmask);
  if (!signature.has_value()) {
    msg("Error: %s\n", signature.error().c_str());
    return;
  }
...
static void SearchSignatureString(std::string input) {
  ...
  // Try to find binary style bitmask like "0b101110" and convert it to a string
  // mask
  ...
  if (!stringMask.empty()) {
    ...
    if (GetRegexMatches(input, std::regex(R"(\\x(?:[0-9A-F]{2}))"),
                        rawByteStrings) &&
        rawByteStrings.size() == stringMask.length()) {
      Signature convertedSignature;
      for (size_t i = 0; const auto &m : rawByteStrings) {
        SignatureByte b{(unsigned char)std::stoi(m.substr(2), nullptr, 16),
                        stringMask[i++] == '?'};
    ...
    else if (GetRegexMatches(input, std::regex(R"((?:0x(?:[0-9A-F]{2}))+)"),
                             rawByteStrings) &&
             rawByteStrings.size() == stringMask.length()) {
      Signature convertedSignature;
      for (size_t i = 0; const auto &m : rawByteStrings) {
    ...
    // Remove whitespace at beginning, questionmarks and spaces at the end, and
    // add one space for the following step
    input = std::regex_replace(input, std::regex("^\\s+"), "");
    input = std::regex_replace(input, std::regex("[? ]+$"), "") + " ";

    // Replace double question marks with single ones to convert x64Dbg style to
    // IDA style We need spaces between signature bytes, because we can not
    // recognize if a signature uses one or two question marks per wildcard
    input = std::regex_replace(input, std::regex(R"(\?\? )"), "? ");

    // Direct match for IDA type signature
    if (std::regex_match(input,
                         std::regex(R"((?:(?:[A-F0-9]{2}\s+)|(?:\?\s+))+)"))) {
      // Just use it
...
static uint32_t WildcardableOperandTypeBitmask =
    BIT(o_reg) | BIT(o_mem) | BIT(o_phrase) | BIT(o_displ) | BIT(o_imm) |
    BIT(o_far) | BIT(o_near) | BIT(o_idpspec0) | BIT(o_idpspec1) |
    BIT(o_idpspec2) | BIT(o_idpspec3) | BIT(o_idpspec4) | BIT(o_idpspec5);
...
bool idaapi plugin_ctx_t::run(size_t) {
  ...
  const char format[] =
      ...
      "Select action:\n" // Title
      "<#Select an address, and create a code signature for "
      "it#Create unique Signature for current code address:R>\n" // Radio Button
                                                                 // 0
      "<#Select an address or variable, and create code signatures "
      "for its references. Will output the shortest 5 "
      "signatures#Find shortest XREF Signature for current data or "
      "code address:R>\n" // Radio Button 1
      "<#Select 1+ instructions, and copy the bytes using the "
      "specified output format#Copy selected code:R>\n" // Radio
                                                        // Button 2
      "<#Paste any string containing your signature/mask and find "
      "matches#Search for a signature:R>>\n" // Radio Button 3

      "Output format:\n" // Title
      "<#Example - E8 ? ? ? ? 45 33 F6 66 44 89 34 33#IDA "
      "Signature:R>\n" // Radio Button 0
      "<#Example - E8 ?? ?? ?? ?? 45 33 F6 66 44 89 34 33#x64Dbg "
      "Signature:R>\n" // Radio Button 1
      "<#Example - "
      "\\xE8\\x00\\x00\\x00\\x00\\x45\\x33\\xF6\\x66\\x44\\x89\\x34"
      "\\x33 x????xxxxxxxx#C Byte Array Signature + String mask : "
      "R>\n" // Radio Button 2
      "<#Example - 0xE8, 0x00, 0x00, 0x00, 0x00, 0x45, 0x33, 0xF6, "
      "0x66, 0x44, 0x89, 0x34, 0x33 0b1111111100001#C Raw Bytes "
      "Signature + Bitmask:R>>\n" // Radio Button 3

      "Options:\n" // Title
      "<#Enable wildcarding for operands, to improve stability of "
      "created signatures#Wildcards for operands:C>\n" // Checkbox
                                                       // Button 0
      "<#Don't stop signature generation when reaching end of "
      "function#Continue when leaving function scope:C>>\n" // Checkbox
                                                            // Button
                                                            // 1
      "<#Configure operand types that should be wildcarded#Operand "
      "types...:B::::>\n"; // Button 0
  ...
  if (ask_form(format, &action, &outputFormat, &options,
    ...
    case 0: {
      ...
      auto signature = GenerateUniqueSignatureForEA(
          ea, wildcardOperands, continueOutsideOfFunction,
    ...
    case 1: {
      // Find XREFs for current selection, generate signatures up to 250 bytes
      // length
      const auto ea = get_screen_ea();
      std::vector<std::tuple<ea_t, Signature>> xrefSignatures;

      show_wait_box("Finding references and generating signatures. This can "
                    "take a while...");

      FindXRefs(ea, wildcardOperands, continueOutsideOfFunction, xrefSignatures,
                250, WildcardableOperandTypeBitmask);
